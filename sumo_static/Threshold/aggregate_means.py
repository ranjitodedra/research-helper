...
def extract_threshold(path: Path) -> int:
    """
    Extracts threshold value from filename.
    Examples:
      5_summary.csv           -> 5
      10_summary.csv          -> 10
      15_summary.csv          -> 15
    ...
    """
    s = path.stem
    # Match number at the start before "_summary" (e.g., "5_summary", "10_summary")
    m = re.search(r"^(\d+)_summary", s, re.IGNORECASE)
    if m:
...
def main():
    rows = []
    # Match files like *_summary.csv (e.g., 5_summary.csv, 10_summary.csv)
