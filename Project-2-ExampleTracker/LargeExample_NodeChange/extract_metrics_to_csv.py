...
Processes subfolders matching *_nodes_ERS_* and writes runtime, travelTime,
distance, energy_consumption, energy_charge_CS, energy_charge_ERS.
"""
...
RE_NODE_FOLDER = re.compile(r"^(\d+)_nodes_ERS")
...
def discover_node_folders(base_dir: Path) -> list[tuple[int, Path]]:
    """Find subdirs matching N_nodes_ERS_* and return (node_count, path) sorted by N."""
    pairs: list[tuple[int, Path]] = []
    for p in base_dir.iterdir():
        if not p.is_dir():
            continue
        m = RE_NODE_FOLDER.match(p.name)
        if m:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Extract metrics from GA/AC/CW output txt files to CSV."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=None,
        help="Root directory containing *_nodes_ERS_* subfolders (default: script dir)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    if not pairs:
        logging.warning("No *_nodes_ERS_* subfolders found in %s", input_dir)
        return

    # Collect one row per node: node -> { GA: parsed, AC: parsed, CW: parsed }
    rows_by_metric: list[list[dict]] = [
        [] for _ in CSV_METRIC_KEYS
    ]  # each list will get one dict per row

    for node, folder in pairs:
    ...
    for csv_name, rows in zip(CSV_NAMES, rows_by_metric):
        ...
        with open(out_path, "w", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(f, fieldnames=["node", "GA", "AC", "CW"])
            w.writeheader()
            w.writerows(rows)
