...
Processes subfolders named like X_Y (CS config, e.g. 0_15, 0_20, 0_25, 0_30)
...
RE_CS_CONFIG = re.compile(r"^(\d+)_(\d+)$")
...
def discover_cs_config_folders(base_dir: Path) -> list[tuple[str, Path]]:
    """
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Extract metrics from GA/AC/CW output txt files to CSV (CS config folders)."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=None,
        help="Root directory containing X_Y config subfolders (default: script dir)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    pairs = discover_cs_config_folders(input_dir)
    if not pairs:
        logging.warning("No config subfolders (X_Y or with GA_output.txt) found in %s", input_dir)
        return

    # Collect one row per config: cs_config -> { GA: parsed, AC: parsed, CW: parsed }
