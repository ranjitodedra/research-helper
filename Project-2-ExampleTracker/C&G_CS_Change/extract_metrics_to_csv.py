...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Extract metrics from GA/CPLEX output txt files to CSV (CS config folders)."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=None,
        help="Root directory containing X_Y CS config subfolders (default: script dir)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    if not pairs:
        logging.warning(
            "No CS config subfolders (X_Y or with GA_output.txt) found in %s",
            input_dir,
        )
        return
