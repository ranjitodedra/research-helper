...
# Files are named like "3_modules.txt"
FILE_PATTERN = re.compile(r"(\d+)_modules\.txt", re.IGNORECASE)
...
def parse_file_info(path: Path):
    """Return module count parsed from filename."""
...
def process_all_files(base_dir: Path):
    """Process all module output files and collect metrics."""
    results = []

    for file_path in sorted(base_dir.glob("*_modules.txt")):
        module_count = parse_file_info(file_path)
...
def write_csv(results, output_dir: Path):
    ...
    ordered = sorted(results, key=lambda item: item["modules"])

    for metric_key, filename in metrics:
        csv_path = output_dir / filename
        with csv_path.open("w", newline="", encoding="utf-8") as handle:
            writer = csv.writer(handle)
            writer.writerow(["modules", metric_key])
            for entry in ordered:
                writer.writerow([entry["modules"], entry.get(metric_key, "")])
        print(f"Wrote {filename}")


if __name__ == "__main__":
    base_path = Path(__file__).parent
    print("Extracting metrics for ModulesChange...")
