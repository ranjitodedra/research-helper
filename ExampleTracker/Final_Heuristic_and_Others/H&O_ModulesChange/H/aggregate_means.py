...
def extract_modules(path: Path) -> int:
    """
    Extracts module count from filename.
    Examples:
      3module_summary.csv   -> 3
      4module_summary.csv   -> 4
      5module_summary.csv   -> 5
      6module_summary.csv   -> 6
    """
    s = path.stem
...
def summarize_one(csv_path: Path) -> dict:
    ...
    modules = extract_modules(csv_path)
...
def main():
    rows = []
    for csv_path in sorted(INPUT_DIR.glob("*module_summary.csv")):
