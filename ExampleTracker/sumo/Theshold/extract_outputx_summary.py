#!/usr/bin/env python3
"""
Extract summary statistics from SUMO routing output files in Theshold directory
and export to CSV format.

Processes numbered .txt files (e.g., 5.txt, 10.txt, 15.txt, 20.txt)
...
def process_directory(input_dir: Path) -> List[Dict[str, float]]:
    """
    Process all numbered .txt files in the input directory.
    ...
    # Find all numbered .txt files (e.g., 5.txt, 10.txt, 15.txt, 20.txt)
    txt_files = sorted(
        [f for f in input_dir.glob("*.txt") if f.name.replace('.txt', '').isdigit()],
        key=lambda p: int(p.stem)
    )
    
    if not txt_files:
        print(f"Error: No numbered .txt files found in {input_dir}", file=sys.stderr)
        print("Expected files like: 5.txt, 10.txt, 15.txt, 20.txt", file=sys.stderr)
        sys.exit(1)
    
    print(f"Found {len(txt_files)} SUMO routing output files")
    
    all_data = []
    
    # Process files in sorted order (by threshold number)
    for run_num, output_file in enumerate(txt_files, start=1):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Extract summary statistics from SUMO routing output files in Theshold directory'
    )
    parser.add_argument(
        '--input-dir',
        type=str,
        default='.',
        help='Input directory containing numbered .txt files (default: current directory)'
    )
    parser.add_argument(
        '--output',
        type=str,
