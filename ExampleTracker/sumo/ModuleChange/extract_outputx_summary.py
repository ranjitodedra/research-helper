#!/usr/bin/env python3
"""
Extract summary statistics from SUMO routing output files in ModuleChange directory
and export to CSV format.

Processes numbered .txt files (e.g., 3.txt, 4.txt, 5.txt, 6.txt)
...
def process_directory(input_dir: Path) -> List[Dict[str, float]]:
    ...
    # Find all numbered .txt files (e.g., 3.txt, 4.txt, 5.txt, 6.txt)
    txt_files = sorted(
    ...
    if not txt_files:
        ...
        print("Expected files like: 3.txt, 4.txt, 5.txt, 6.txt", file=sys.stderr)
        sys.exit(1)
    ...
    # Process files in sorted order (by module number)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Extract summary statistics from SUMO routing output files in ModuleChange directory'
    )
    parser.add_argument(
        '--input-dir',
        type=str,
