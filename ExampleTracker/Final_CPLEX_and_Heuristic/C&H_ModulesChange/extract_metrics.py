...
# Extract module count from folder names like "6c_3bss_16total_C&H_ModulesChange_6_Module"
FOLDER_PATTERN = re.compile(r"_(\d+)_Module", re.IGNORECASE)
...
def parse_folder_info(folder_name: str):
    """Return module count from folder name."""
...
def process_all(base_dir: Path):
    ...
    for folder in sorted(base_dir.iterdir()):
        ...
        module_count = parse_folder_info(folder.name)
...
def write_csvs(results: dict, output_dir: Path):
    ...
    # Extract unique module counts for ordering
    module_counts = sorted({key[0] for metric in results.values() for key in metric})
    ...
    for metric_name, filename in csv_files.items():
        ...
        with path.open("w", newline="", encoding="utf-8") as handle:
            ...
            writer.writerow(["module"] + algo_order)
            for module in module_counts:
                row = [module]
                for algo in algo_order:
                    row.append(results[metric_name].get((module, algo), ""))
                writer.writerow(row)
