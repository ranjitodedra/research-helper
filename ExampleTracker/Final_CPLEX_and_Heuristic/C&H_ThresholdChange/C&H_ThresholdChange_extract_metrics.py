...
# Extract threshold value from folder names like "6c_3bss_16total_C&H_ThresholdChange_5"
FOLDER_PATTERN = re.compile(r"_(\d+)$", re.IGNORECASE)
...
def parse_folder_info(folder_name: str):
    """Return threshold value from folder name."""
...
def process_all(base_dir: Path):
    ...
    for folder in sorted(base_dir.iterdir()):
        ...
        threshold = parse_folder_info(folder.name)
...
def write_csvs(results: dict, output_dir: Path):
    ...
    # Extract unique threshold values for ordering
    thresholds = sorted({key[0] for metric in results.values() for key in metric})
    ...
    for metric_name, filename in csv_files.items():
        ...
        with path.open("w", newline="", encoding="utf-8") as handle:
            ...
            writer.writerow(["threshold"] + algo_order)
            for threshold in thresholds:
                row = [threshold]
                for algo in algo_order:
                    row.append(results[metric_name].get((threshold, algo), ""))
                writer.writerow(row)
