...
# Traffic level order for consistent CSV ordering
TRAFFIC_ORDER = {"Low": 1, "Mid": 2, "High": 3}

# Glob patterns to find per-algorithm outputs
FILE_PATTERNS = {
    "CPLEX": "*CPLEX_output*.txt",
    "H": "*H_output*.txt",
}
...
def process_all(base_dir: Path):
    """Process CH folder (Low/Mid/High) and collect metrics."""
    results = {
        "travelTime": {},
        "energy": {},
        "distance": {},
    ...
    for traffic_folder in sorted(base_dir.iterdir()):
        ...
        for algo, pattern in FILE_PATTERNS.items():
            matches = list(traffic_folder.glob(pattern))
            if not matches:
                print(f"Warning: No file matching {pattern} in {traffic_folder.name}")
                continue
            ...
            print(f"Processed {traffic_folder.name}/{file_path.name}")
...
if __name__ == "__main__":
    ...
    print("Extracting metrics for CPLEX and Heuristic CH scenarios...")
